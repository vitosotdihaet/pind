...
def find_n_keys_by_fk_in_index_comparison_count(
    number_of_keys: int, tree_order: int
) -> int:
    return ceil(log(number_of_keys, tree_order)) * ceil(
        log(max_key_count_in_node(tree_order))
    )


def find_n_keys_by_fk_in_index_memory_jumps_count(
    ...
    return (
        ...
        + ceil(
            ...
            / max_key_count_in_node(tree_order)
        )
        + 1
    )
