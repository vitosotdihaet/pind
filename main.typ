...
#theorem(name: [о времени ожидания исполнения запроса координатором при поиске в ГВИ])[
  ...
  - $#fk_execute_request_time$ и $#fk_execute_response_time$ --- времена задержки передачи запроса между координатором и исполнителем со вторичным ключом по сети;
  - $#service_time_fk_execute_search_GSI$ --- время ожидания ответа от исполнителя со вторичным ключом;
  - $#exec_plan_size$ --- размер плана исполнения;
  - $#cluster_net_speed$ --- скорость передачи данных между узлами СУБД;
  - $#pk_per_fk_cardinality_per_replicaset#replicaset_count$ --- количество записей с искомым вторичным ключом в кластере;
  - $#row_size$ --- размер одной записи.
