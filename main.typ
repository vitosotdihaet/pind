...
#lemma(name: [о времени ожидания пользователя при запросе в СУБД Picodata])[
  ...
  - $#query_size$ --- размер пользовательского запроса;
...
#theorem(name: [о времени ожидания ответа от исполнителя с искомым вторичным ключом])[
  ...
  - $#queue_time_execute_search_GSI$ --- время ожидания запроса в очереди исполнителя;
  - $#time_fk_execute_search_GSI$ --- время поиска первичных ключей в исполнителе со вторичными ключами;
  - $#time_fk_execute_search_GSI_ith$ --- время ожидания ответа от исполнителя под номером $i$;
  - $#cluster_request_time_ith$ и $#cluster_response_time_ith$ --- времена задержки передачи запроса между координатором и исполнителем под номером $i$ по сети;
  - $#exec_plan_size_ith$ --- размер плана исполнения для исполнителя под номером $i$;
  - $#cluster_net_speed$ --- скорость передачи данных между узлами СУБД;
  - $#service_time_execute_search_GSI$ --- время ожидания обработки запроса исполнителем;
  - $#pk_per_fk_cardinality_per_replicaset$ --- количество записей с искомым вторичным ключом на наборе реплик;
  - $#row_size$ --- размер одной записи;
  - $k <= #replicaset_count$ --- количество наборов реплик, на которых распологаются первичные ключи.
]
...
- $#random_part_of_tus_GSI tilde Gamma(2#user_time_k, #user_time_theta)$ --- сетевые задержки между пользователем и координатором;
- $#random_part_of_tce_GSI tilde Gamma(2#cluster_time_k, #cluster_time_theta)$ --- сетевые задержки между координатором и исполнителем со вторичным ключом;
- $#random_part_of_tfe_GSI$ --- максимум $k$ независимых одинаково распределённых случайных величин с функцией распределения $F_#random_part_of_tfe_GSI (t)$.
...
$
  #queue_time_total_GSI = 3 #queue_time_execute_search_GSI,
...
- поиск ГВИ: $#queue_time_total_GSI = 3 #queue_time_execute_search_GSI$;
