"""
Taken and modified from https://github.com/xingjian-zhang/typst2img.git
"""
...
def render_helper(formula, src_output_path, img_output_path, fmt="svg", preview=True):
    ...
    def render(
        ...
        if name is not None:
            ...
            if img_output_path is not None:
                src_output_path = (
                    src_output_path or os.path.splitext(img_output_path)[0] + ".typ"
                )
            elif not preview:
                raise ValueError(
                ...
                img_output_path = "output.svg"
        src = self.template + f"\n#display[{formula}]"
        render_helper(src, src_output_path, img_output_path, fmt=fmt, preview=preview)


def get_typst_context(
    filename: Path, find_from: str | None = None, find_to: str | None = None
) -> str:
    out = ""

    with open(filename, "r") as t:
        reading = find_from is None
        line: str = t.readline()
        while line:
            if find_to and find_to in line:
                break
            if reading or find_from in line:  # pyright: ignore[reportOperatorIssue]
                reading = True
                out += line
            line = t.readline()
